...
def train_sequence(model, sequence, sequence_sum):
	...
	for j in range(0, sequence_sum):
		inpi = inp[:, j, :, :, :]
		gti = target[:, j, :, :, :]
		rai = filters.median.median_blur(raw_final[:, j, :, :, :], (3, 3))
		...
		# print(rai.shape) # torch.Size([1, 8, 576, 960])
	...
	for j in range(0, sequence_sum):
		...
		l, ls, lg, lt, lm = loss_func(output, t_output, target, t_target, raw)
		loss_final += k_temp * l
		ls_final +=  k_temp * ls
		lg_final += k_temp * lg
		lt_final += k_temp * lt
	...
	return loss_final, ls_final, lg_final, lt_final, lm_final
...
def train(model, dataset, optimizer, epoch, now, sequence_sum):
	...
	for i, item in enumerate(dataset):
		optimizer.zero_grad()
		loss_final, ls_final, lg_final, lt_final, lm_final = train_sequence(model, item, sequence_sum)
		...
		if i % 50 == 0:
			...
			print('[Epoch : %s] [%s/%s] Loss => %s , L1 => %s , HFEN => %s, L_mean => %s' %
					(epoch+1, (i+1), len(dataset), loss_final.item(), ls_final.item(),
						lg_final.item(), lm_final.item()))
			sys.stdout.flush()
