...
class RAEData(Dataset):
	def __init__(self, input_dir, sequence_sum):
		super(RAEData, self).__init__()
		self.input_dir = input_dir
		self.sequence_sum = sequence_sum
	...
	def __getitem__(self, index):
		...
		A, B, C, ALBEDO = [], [], [], []
		# batch size = 7
		for i in range(index*self.sequence_sum, (index+1)*self.sequence_sum):
			...
			C_i = np.expand_dims(shading, axis=0)
			B.append(B_i)
			C.append(C_i)
		...
		return {
			...
			'C': C.type(torch.float).to('cuda:0'),
			'ALBEDO': ALBEDO.type(torch.float).to('cuda:0')
		} # A 通道，B参考，C原图片通道，ALBEDO：ALBEDO

		
	def __len__(self):
		# reserve one batch for test
		return self.length//self.sequence_sum - 1
