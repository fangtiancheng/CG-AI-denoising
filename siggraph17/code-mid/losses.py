...
def loss_func(output, temporal_output, target, temporal_target, raws):

	lm = l1_norm(output, raws)
	ls = l1_norm(output, target)
	lg = HFEN(output, target)
	# lt = temporal_norm(temporal_output, temporal_target)

	# return 0.8 * ls + 0.2 * lg + 0.05 * lt, ls, lg, lt
	return 0.70 * ls + 0.25 * lg + 0.05 * lm, ls, lg, 0, lm
