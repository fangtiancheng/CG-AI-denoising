...
class RAEData(Dataset):
	def __init__(self, input_dir):
		...
		self.flow = get_paths(input_dir, 'flow')
	...
	def _check(self):
		...
		assert(len(self.flow) == self.length)
		...
		assert (testimg.shape == cv2.imread(self.flow[0]).shape)
	...
	def __getitem__(self, index):
		...
		for i in range(index*7, (index+1)*7):
			...
			flow = cv2.imread(self.flow[i])[::2,::2,:][:self.width, :self.height, :]
			...
			flow = flow.astype(np.float) / 255.0
			...
			A_i = np.concatenate([shading, normal, flow, depth, roughness], axis=2)
		...
		# (7, 576, 960, 11) (7, 576, 960, 3) (7, 576, 960, 3)
